...
df = pd.DataFrame(data=kinetic_energy_mesh, columns=x_values, index=t_values)

# Create an animated surface plot using Plotly
frames = [go.Frame(data=[go.Surface(z=df.iloc[:i+1, :])]) for i in range(len(t_values))]

fig = go.Figure(data=[go.Surface(z=df.values)], frames=frames)
fig.update_layout(
