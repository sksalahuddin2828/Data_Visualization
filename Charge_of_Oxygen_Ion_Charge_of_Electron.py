...
# Visualization
# Create a 3D plot to visualize the circular path of the ion
theta = np.linspace(0, 2 * np.pi, 100)
x = radius * np.cos(theta)
y = radius * np.sin(theta)
z = np.zeros_like(theta)
...
ax.set_zlabel('Z')
ax.legend()
ax.set_title('Circular Path of Oxygen-16 Ion')
...
# Add explanations, annotations, and text labels to the plots to make them informative and creative.

# Save the figures if needed
# plt.savefig('physics_project.png')
