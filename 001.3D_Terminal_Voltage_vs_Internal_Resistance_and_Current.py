...
R_internal_mesh, I_mesh = np.meshgrid(internal_resistance_values, I_values)
delta_V_mesh = emf - (I_mesh + delta_I) * R_internal_mesh  # Terminal voltage

# Plot terminal voltage as a surface
ax.plot_surface(R_internal_mesh, I_mesh, delta_V_mesh, cmap='viridis', alpha=0.8)

# Add voltage contours
ax.contour(R_internal_mesh, I_mesh, delta_V_mesh, zdir='z', offset=np.min(delta_V_mesh),
           cmap='coolwarm', linewidths=1)

# Add current lines
ax.contour(R_internal_mesh, I_mesh, delta_V_mesh, zdir='y', offset=np.min(I_mesh),
           cmap='coolwarm', linewidths=1)
