...
# Create symbolic variables for mathematical expressions
V, I, R, P = sp.symbols('V I R P')
Ohms_law = sp.Eq(V, I * R)
Power_formula = sp.Eq(P, V * I)

# Solve for terminal voltage symbolically
terminal_voltage_symbolic = sp.solve(Ohms_law.subs({I: current, R: internal_resistance}), V)[0]
...
print("Terminal Voltage (V) =", sp.latex(terminal_voltage_symbolic))
print("Power Produced (W) =", sp.latex(Power_formula.subs({V: emf, I: current})))
print("Power Delivered to Load (W) =", sp.latex(Power_formula.subs({V: terminal_voltage_symbolic, I: current})))
