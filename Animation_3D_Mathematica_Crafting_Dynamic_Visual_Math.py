...
frames = []
for i, n_terms in enumerate(n_terms_vals):
    frame = go.Frame(data=[go.Scatter(x=x_vals, y=tan_x, mode='lines', name='tan(x)'),
                           go.Scatter(x=x_vals, y=series_data[:i+1], mode='lines+markers', name='Series Approximation')],
                     name=f"{n_terms} terms")
    
    # Add annotations
    annotations = [dict(x=2, y=10, text=f"Number of Terms: {n_terms}",
                        showarrow=False, font=dict(size=12))]
    frame.data[1]['text'] = annotations
    frames.append(frame)
...
for frame in frames:
    fig.add_trace(frame.data[1], row=2, col=1)

# Set initial frame
initial_frame = go.Frame(data=[go.Scatter(x=x_vals, y=tan_x, mode='lines', name='tan(x)'),
                               go.Scatter(x=x_vals, y=series_data[:1], mode='lines+markers', name='Series Approximation')],
                         name=f"{n_terms_vals[0]} terms")
