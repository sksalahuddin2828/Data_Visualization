...
def plot_fermat_theorem(limit):
    ...
    # Additional visualization using matplotlib and sympy
    if valid_solution:
        ...
        # Define the equation x^3 + y^3 = z^3
        equation = Eq(X**3 + Y**3, avg_z**3)

        # Solve for y in terms of x and avg_z
        y_sol = solve(equation, Y)

        # Convert the solution to a lambda function for plotting
        y_func = lambda x_val: y_sol[0].subs(X, x_val).evalf()

        # Generate x values for plotting
        x_vals = np.linspace(min(x), max(x), 100)

        # Calculate corresponding y values using the lambda function
        y_vals = np.array([y_func(x_val) for x_val in x_vals])

        # Plot the curve y = f(x) for x^3 + y^3 = z^3
        plt.figure(figsize=(8, 6))
        plt.plot(x_vals, y_vals, label=r'$y = f(x)$', color='green')
        plt.scatter(x, y, s=20, color='red', label='Valid Solutions')
        plt.xlabel('x', fontsize=14)
        plt.ylabel('y', fontsize=14)
        plt.title('Curve for x^3 + y^3 = z^3', fontsize=16)
