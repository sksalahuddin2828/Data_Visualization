...
def plot_bridge_balance(R3_value):
    ...
    # Plot a sinewave and its relationship to the bridge
    t = np.linspace(0, 2 * np.pi, 100)
    voltage_waveform = voltage * np.sin(t)
    ax4.plot(t, voltage_waveform, color='royalblue')
    ax4.set_xlabel('Time')
    ax4.set_ylabel('Voltage (V)')
    ax4.set_title('Voltage vs. Time (Sine Waveform)')
